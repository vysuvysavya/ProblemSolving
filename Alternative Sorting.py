...
class Solution:
    def alternateSort(self, arr):
        ...
        # Result list to store the alternating sequence
        result = []
        
        # Use two pointers: one at the beginning and one at the end of the array
        i, j = 0, len(arr) - 1
        
        # Alternate picking elements from end and start
        while i <= j:
            if i != j:
                result.append(arr[j])  # Pick the largest remaining
                result.append(arr[i])  # Pick the smallest remaining
            else:
                result.append(arr[i])  # Add the last middle element if i == j
            i += 1
            j -= 1
